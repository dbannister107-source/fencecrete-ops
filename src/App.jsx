...
const SS = { contract_review:'Review', production_queue:'Prod Queue', in_production:'In Prod', inventory_ready:'Inventory', active_install:'Active Install', fence_complete:'Fence Done', fully_complete:'Fully Complete', closed:'Closed' };
...
function BillingPage({jobs,onRefresh,onNav}){
  ...
  const cycleActiveJobs=useMemo(()=>jobs.filter(j=>['in_production','inventory_ready','active_install','fence_complete','fully_complete'].includes(j.status)),[jobs]);
...
function ProductionPage({jobs,onRefresh}){
  ...
  const move=async(job,ns)=>{if(!editUnlocked){console.warn('[Kanban] Move blocked — editing locked');return;}const u={status:ns};const today=new Date().toISOString().split('T')[0];if(ns==='active_install')u.active_install_date=today;if(ns==='fence_complete')u.fence_complete_date=today;if(ns==='fully_complete')u.fully_complete_date=today;if(ns==='closed')u.closed_date=today;console.log('[Kanban] Moving',job.job_name,'('+job.id+') from',job.status,'→',ns,'body=',JSON.stringify(u));try{const res=await fetch(`${SB}/rest/v1/jobs?id=eq.${job.id}`,{method:'PATCH',headers:{apikey:KEY,Authorization:`Bearer ${KEY}`,'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify(u)});const txt=await res.text();console.log('[Kanban] PATCH response: status=',res.status,'body=',txt.substring(0,200));if(!res.ok){setMoveToast({msg:`Move failed (${res.status}): ${txt}`,ok:false});return;}fireAlert('job_updated',{...job,...u});logAct(job,'status_change','status',job.status,ns);setMoveToast({msg:`Moved ${job.job_name} to ${SL[ns]||ns}`,ok:true});onRefresh();}catch(e){console.error('[Kanban] Move error:',e);setMoveToast({msg:e.message||'Move failed',ok:false});}};
