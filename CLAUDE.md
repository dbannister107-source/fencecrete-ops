...
10. ⚠️ **Duplicated design tokens** — partially resolved 2026-05-03: `src/shared/ui.js` created with token vocabulary (`COLOR`, `FONT`, `RADIUS`, `SHADOW`) + canonical style objects (`card`, `btnP`, `btnS`, `btnG`, `btnB`, `inputS`, `stat`, `statLabel`, `statValue`). Migrated CustomerMasterPage (full), ContractsWorkbenchPage (partial — `btnS` is a smaller-variant kept local), SpecialtyVisitsPage (partial — `btnS` and `inputS` variants kept local). **Not migrated** (intentional variants or older shape; defer until design alignment): App.jsx module-scope (`8px/16px` shape, line 546-549), SystemEventsPage / SharePointLinksPage (large variants), MyPlatePage (custom small + zero-padding card), PISFormPage (PIS portal styling). New work should `import { ... } from 'shared/ui'` rather than defining locally.
