...
const STS = ['contract_review','production_queue','in_production','ready_to_install','in_install','complete'];
const SL = { contract_review:'Contract Review', production_queue:'Production Queue', in_production:'In Production', ready_to_install:'Ready to Install', in_install:'In Install', complete:'Complete' };
const SC = { contract_review:'#B45309', production_queue:'#1D4ED8', in_production:'#6D28D9', ready_to_install:'#7C3AED', in_install:'#2563EB', complete:'#065F46' };
const SB_ = { contract_review:'#FEF3C7', production_queue:'#DBEAFE', in_production:'#EDE9FE', ready_to_install:'#F3E8FF', in_install:'#DBEAFE', complete:'#D1FAE5' };
const SS = { contract_review:'Review', production_queue:'Prod Queue', in_production:'In Prod', ready_to_install:'Ready', in_install:'Install', complete:'Complete' };
...
const NEXT_STATUS = { contract_review:'production_queue', production_queue:'in_production', in_production:'ready_to_install', ready_to_install:'in_install', in_install:'complete' };
...
const DEF_VIS=['status','market','job_number','sharepoint','job_name','customer_name','fence_type','sales_rep','pm','adj_contract_value','left_to_bill','pct_billed','total_lf','contract_date','est_start_date','last_billed','aia_billing','bonds','certified_payroll','ocip_ccip','third_party_billing','notes'];
  ...
  return(<div style={{position:'fixed',inset:0,background:'#F4F4F2',zIndex:250,display:'flex',flexDirection:'column'}}>
    ...
    <div style={{flex:1,overflow:'auto',padding:'20px 24px'}}>
      {sec==='info'&&<div style={{display:'grid',gridTemplateColumns:grd,gap:12}}>
        ...
        <div>{fLbl('Status')}<select value={f.status} onChange={e=>set('status',e.target.value)} style={inputS}>{[['contract_review','Contract Review'],['production_queue','Booked-Not Started'],['in_production','In Production'],['complete','Complete']].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
...
function Dashboard({jobs,onNav}){
  ...
  return(<div>
    ...
    {/* Quick stats */}
    {(()=>{const rtiCount=jobs.filter(j=>j.status==='ready_to_install').length;const iiCount=jobs.filter(j=>j.status==='in_install').length;const collMo=jobs.filter(j=>j.collected&&j.collected_date&&new Date(j.collected_date).getMonth()===now.getMonth()&&new Date(j.collected_date).getFullYear()===now.getFullYear()).length;const outstanding=jobs.filter(j=>j.status==='complete'&&!j.collected).length;return<div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10,marginBottom:24}}>{[['Active',active.length],['Ready to Install',rtiCount],['In Install',iiCount],['Collected Mo.',collMo],['Outstanding',outstanding],['Completed Mo.',compThisMonth],['Avg Contract',$k(active.length?tc/active.length:0)],['Largest',largest?largest.job_name?.slice(0,18):'—'],['Oldest Unbilled',oldestUnbilled?`${oldestUnbilled.contract_age}d`:'—'],['Total',jobs.length]].map(([l,v])=><div key={l} style={{background:'#F9F8F6',border:'1px solid #E5E3E0',borderRadius:8,padding:'8px 12px'}}><div style={{fontFamily:'Inter',fontWeight:700,fontSize:14,color:'#1A1A1A'}}>{v}</div><div style={{fontSize:10,color:'#9E9B96'}}>{l}</div></div>)}</div>;})()}
    {/* Backlog Health */}
    {(()=>{
    ...
    <div style={{position:'sticky',top:0,zIndex:10,background:'#F4F4F2',paddingBottom:8,marginBottom:8}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button onClick={()=>setShowNewForm(true)} style={{...btnP,background:'#065F46'}}>+ New Project</button>
...
function BillingPage({jobs,onRefresh}){
  ...
  // Invoice a single PM entry — use amount_this_period from the entry, accumulate retainage
  const invoiceEntry=async()=>{if(!invoiceModal)return;const e=invoiceModal;const job=jobs.find(j=>j.id===e.job_id);const today=new Date().toISOString().split('T')[0];const amt=n(invoiceAmt)||n(e.net_amount_due)||n(e.amount_this_period);const retAmt=n(e.retainage_amount);await sbPatch('pm_billing_entries',e.id,{status:'invoiced',invoiced_by:'Accounting',invoiced_date:today});if(job){const u={last_billed:today};if(amt>0){const newYTD=n(job.ytd_invoiced)+amt;const adj=n(job.adj_contract_value||job.contract_value);u.ytd_invoiced=newYTD;u.pct_billed=adj>0?Math.round(newYTD/adj*10000)/10000:0;u.left_to_bill=adj-newYTD;}if(retAmt>0)u.retainage_held=n(job.retainage_held)+retAmt;await sbPatch('jobs',job.id,u);fireAlert('billing_logged',{...job,...u});logAct(job,'billing_update','ytd_invoiced',job.ytd_invoiced,u.ytd_invoiced||job.ytd_invoiced);}setInvoiceModal(null);setInvoiceAmt('');setToast('Entry invoiced');fetchEntries();onRefresh();};
...
/* ═══ PM BILLING PAGE ═══ */
const ACTIVE_STATUSES=['in_production','ready_to_install','in_install','complete'];
...
function ProductionPage({jobs,onRefresh}){
  ...
  const move=async(job,ns)=>{const u={status:ns};const today=new Date().toISOString().split('T')[0];if(ns==='ready_to_install')u.ready_to_install_date=today;if(ns==='in_install')u.in_install_date=today;if(ns==='complete')u.complete_date=today;await sbPatch('jobs',job.id,u);fireAlert('job_updated',{...job,...u});logAct(job,'status_change','status',job.status,ns);onRefresh();};
  ...
  const pipeLF=filtered.filter(j=>['production_queue','in_production','ready_to_install','in_install'].includes(j.status)).reduce((s,j)=>s+n(j.total_lf),0);
...
function ReportsPage({jobs}){
  ...
  const renderReport=()=>{
    ...
    if(activeRpt==='outstanding'){const oc=jobs.filter(j=>j.status==='complete'&&!j.collected).sort((a,b)=>{const ad=a.complete_date?Math.round((Date.now()-new Date(a.complete_date).getTime())/86400000):0;const bd=b.complete_date?Math.round((Date.now()-new Date(b.complete_date).getTime())/86400000):0;return bd-ad;});const totalOut=oc.reduce((s,j)=>s+n(j.left_to_bill),0);return<div><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{borderBottom:'2px solid #E5E3E0'}}>{['Project','Job #','PM','Market','Contract','Left to Bill','Complete Date','Days Since'].map(h=><th key={h} style={{textAlign:'left',padding:8,color:'#6B6056',fontWeight:600,fontSize:11,textTransform:'uppercase'}}>{h}</th>)}</tr></thead><tbody>{oc.map(j=>{const days=j.complete_date?Math.round((Date.now()-new Date(j.complete_date).getTime())/86400000):0;return<tr key={j.id} style={{borderBottom:'1px solid #F4F4F2'}}><td style={{padding:'6px 8px',fontWeight:500,maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{j.job_name}</td><td style={{padding:'6px 8px'}}>{j.job_number||'—'}</td><td style={{padding:'6px 8px'}}>{j.pm||'—'}</td><td style={{padding:'6px 8px'}}><span style={pill(MC[j.market]||'#6B6056',MB[j.market]||'#F4F4F2')}>{MS[j.market]||'—'}</span></td><td style={{padding:'6px 8px',fontFamily:'Inter',fontWeight:700}}>{$(j.adj_contract_value||j.contract_value)}</td><td style={{padding:'6px 8px',fontFamily:'Inter',fontWeight:700,color:'#991B1B'}}>{$(j.left_to_bill)}</td><td style={{padding:'6px 8px'}}>{fD(j.complete_date)}</td><td style={{padding:'6px 8px',fontWeight:700,color:days>90?'#991B1B':days>30?'#B45309':'#6B6056'}}>{days}d</td></tr>;})}</tbody></table>{oc.length===0&&<div style={{padding:20,textAlign:'center',color:'#9E9B96'}}>No outstanding collections</div>}<div style={{marginTop:12,padding:12,background:'#F9F8F6',borderRadius:8,fontFamily:'Inter',fontWeight:700}}>Total Outstanding: <span style={{color:'#991B1B'}}>{$(totalOut)}</span> across {oc.length} jobs</div></div>;}
    return null;
  };
...
function PMDailyReportPage({jobs}){
  ...
  return(<div>
    ...
    {selPM&&tab==='new'&&<div style={{...card,maxHeight:'calc(100vh - 260px)',overflow:'auto',paddingBottom:80}}>
      ...
      {/* Auto-populated market display */}
      {selJobId&&(()=>{const sj=jobs.find(j=>j.id===selJobId);return sj?<div style={{display:'flex',gap:12,marginBottom:12,fontSize:12,color:'#6B6056'}}><span>Market: <span style={pill(MC[sj.market]||'#6B6056',MB[sj.market]||'#F4F4F2')}>{MS[sj.market]||sj.market||'—'}</span></span><span>Fence Type: <strong>{sj.fence_type||'—'}</strong></span></div>:null;})()}
      ...
      {/* Submit */}
      <div style={{position:'sticky',bottom:0,background:'#FFF',padding:'16px 0 4px',marginTop:20,borderTop:'1px solid #E5E3E0'}}><button onClick={submitReport} style={{...btnP,width:'100%',padding:'14px 0',fontSize:16,minHeight:48}}>Submit Report</button></div>
    </div>}
  </div>);
}
