...
- **App.jsx fetch migration Phase 2A + 2B** [`1f9b6d6`, `ae3f2d2`]. Foundation: replaced 4 local `sbGet/sbPatch/sbPost/sbDel` wrappers with imports from `shared/sb.js`; added `sbAuth*` namespace covering 6 GoTrue endpoints; migrated 6 auth functions + 23 functions/v1 fetches → `sbFn` + 5 storage/v1 fetches → `sbStorage*` helpers. **38 of 113 no-restricted-syntax warnings retired (33% drop, 113 → 75)**. The H constant kept as transitional shim; ~75 inline rest/v1 fetches remain for Phase 2C tomorrow. App.jsx ~85 lines lighter for it (manual error handling collapsed into helper-internal `_check`).
...
6. ⚠️ **No API abstraction — direct `fetch()` in App.jsx**. Phase 1 done 2026-05-03 (sb.js as canonical entry + ESLint `no-restricted-syntax` rule). Phase 2A + 2B further done 2026-05-03 (evening): retired 38 of 113 warnings (113 → 75; 33% drop) by replacing the 4 local sb wrappers, adding `sbAuth*` namespace + migrating 6 auth functions, migrating 23 functions/v1 fetches → `sbFn`, and migrating 5 storage/v1 fetches → `sbStorage*`. **Phase 2C remaining**: ~75 inline rest/v1 fetches with mixed shapes. Each is a small mechanical replacement (sbGet for query+headers H, sbPost for body, sbPatchWhere for non-id filters, etc.) but each requires reading the surrounding context for custom Prefer headers, returnMinimal opts, etc. Estimated 3-4 hours to clear remaining; can be done incrementally without breaking anything since the lint rule warns rather than errors on App.jsx (per `eslintConfig.overrides`). The `H` constant in App.jsx is kept as a transitional shim — it goes when the count hits zero.
