...
function ProductionPage({jobs,onRefresh}){
  ...
  const move=async(job,ns)=>{if(!editUnlocked)return;const u={status:ns};const today=new Date().toISOString().split('T')[0];if(ns==='ready_to_install')u.ready_to_install_date=today;if(ns==='in_install')u.in_install_date=today;if(ns==='complete')u.complete_date=today;await sbPatch('jobs',job.id,u);fireAlert('job_updated',{...job,...u});logAct(job,'status_change','status',job.status,ns);onRefresh();};
