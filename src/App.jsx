...
const COL_GROUPS=[
  ...
  {label:'Product',keys:['fence_type','primary_fence_type','fence_addons','total_lf_precast','total_lf_masonry','total_lf_wrought_iron','number_of_gates','total_lf','lf_precast','lf_single_wythe','lf_wrought_iron','style','color','height_precast']},
  ...
  return(<div>
    ...
    {projTab==='closed'&&<div style={{...card,padding:0,overflow:'auto',maxHeight:'calc(100vh - 320px)'}}>
      ...
      <tbody>{closedJobs.map((j,i)=><tr key={j.id} onClick={()=>{setEditJob(j);setIsNew(false);}} style={{cursor:'pointer',background:i%2===0?'#FFF':'#FAFAF8',color:'#6B6056'}} onMouseEnter={e=>e.currentTarget.style.background='#FDF9F6'} onMouseLeave={e=>e.currentTarget.style.background=i%2===0?'#FFF':'#FAFAF8'}>
        ...
        <td style={{padding:'8px 10px'}}>{lfSW(j)>0?lfSW(j).toLocaleString():<span style={{color:'#9E9B96'}}>—</span>}</td>
        <td style={{padding:'8px 10px'}}>{lfWI(j)>0?lfWI(j).toLocaleString():<span style={{color:'#9E9B96'}}>—</span>}</td>
...
const NAV_GROUPS=[
  ...
  {label:'PROJECTS',items:[{key:'projects',label:'Projects',icon:'📋'},{key:'production',label:'Production Board',icon:'🗂'}]},
