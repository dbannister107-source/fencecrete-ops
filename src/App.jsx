...
const STS = ['contract_review','production_queue','in_production','active_installation','installation_complete','complete','closed'];
const SL = { contract_review:'Contract Review', production_queue:'Production Queue', in_production:'In Production', active_installation:'Active Installation', installation_complete:'Installation Complete', complete:'Complete', closed:'Closed' };
const SC = { contract_review:'#B45309', production_queue:'#1D4ED8', in_production:'#6D28D9', active_installation:'#B45309', installation_complete:'#0F6E56', complete:'#065F46', closed:'#FFFFFF' };
const SB_ = { contract_review:'#FEF3C7', production_queue:'#DBEAFE', in_production:'#EDE9FE', active_installation:'#FEF3C7', installation_complete:'#D1FAE5', complete:'#D1FAE5', closed:'#374151' };
const SS = { contract_review:'Review', production_queue:'Prod Queue', in_production:'In Prod', active_installation:'Active Install', installation_complete:'Install Done', complete:'Complete', closed:'Closed' };
...
const NEXT_STATUS = { contract_review:'production_queue', production_queue:'in_production', in_production:'active_installation', active_installation:'installation_complete', installation_complete:'complete', complete:'closed' };
  ...
  return(<div style={{position:'fixed',inset:0,background:'#F4F4F2',zIndex:250,display:'flex',flexDirection:'column'}}>
    ...
    <div style={{flex:1,overflow:'auto',padding:'20px 24px'}}>
      ...
      {sec==='info'&&<div style={{display:'grid',gridTemplateColumns:grd,gap:12}}>
        ...
        <div>{fLbl('Status')}<select value={f.status} onChange={e=>set('status',e.target.value)} style={inputS}>{[['contract_review','Contract Review'],['production_queue','Booked-Not Started'],['in_production','In Production'],['active_installation','Active Installation'],['installation_complete','Installation Complete'],['complete','Complete'],['closed','Closed']].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
...
function BillingPage({jobs,onRefresh,onNav}){
  ...
  const cycleActiveJobs=useMemo(()=>jobs.filter(j=>['in_production','active_installation','installation_complete','complete'].includes(j.status)),[jobs]);
...
function ProdCard({j,move,locked}){const ns=NEXT_STATUS[j.status];return<div style={{...card,padding:12,marginBottom:6}}><div style={{fontWeight:600,fontSize:13,marginBottom:4,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{j.job_name}</div><div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:4}}><span style={pill(MC[j.market]||'#6B6056',MB[j.market]||'#F4F4F2')}>{MS[j.market]||'—'}</span><span style={pill(SC[j.status]||'#6B6056',SB_[j.status]||'#F4F4F2')}>{SS[j.status]}</span>{j.sales_rep&&<span style={{fontSize:10,color:'#6B6056',background:'#F4F4F2',padding:'1px 5px',borderRadius:4}}>{j.sales_rep}</span>}</div><div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#6B6056',marginBottom:2}}><span>{n(j.total_lf).toLocaleString()} LF</span><span style={{fontFamily:'Inter',fontWeight:700,color:'#8B2020'}}>{$(j.adj_contract_value||j.contract_value)}</span></div>{j.est_start_date&&<div style={{marginBottom:2}}><StartDateBadge date={j.est_start_date} status={j.status}/></div>}{j.contract_age&&<div style={{fontSize:10,color:'#9E9B96'}}>{j.contract_age}d old</div>}{!locked&&<div style={{display:'flex',gap:4,marginTop:6}}>{ns&&<button onClick={()=>move(j,ns)} style={{flex:2,padding:'5px 4px',borderRadius:6,border:`1px solid ${SC[ns]}40`,background:SB_[ns],color:SC[ns],fontSize:10,fontWeight:700,cursor:'pointer'}}>→ {SS[ns]}</button>}<select onChange={e=>{if(e.target.value)move(j,e.target.value);e.target.value='';}} style={{flex:1,padding:'4px',borderRadius:6,border:'1px solid #E5E3E0',fontSize:10,color:'#6B6056',cursor:'pointer',background:'#FFF'}}><option value="">More...</option>{STS.filter(s=>s!==j.status&&s!==ns).map(s=><option key={s} value={s}>{SS[s]}</option>)}</select></div>}</div>;}
...
function ProductionPage({jobs,onRefresh}){
  ...
  const move=async(job,ns)=>{if(!editUnlocked)return;const u={status:ns};const today=new Date().toISOString().split('T')[0];if(ns==='active_installation')u.active_installation_date=today;if(ns==='installation_complete')u.installation_complete_date=today;if(ns==='complete')u.complete_date=today;if(ns==='closed')u.closed_date=today;await sbPatch('jobs',job.id,u);fireAlert('job_updated',{...job,...u});logAct(job,'status_change','status',job.status,ns);onRefresh();};
  const filtered=useMemo(()=>{let f=jobs.filter(j=>j.status!=='closed'&&j.status!=='contract_review');if(mktF)f=f.filter(j=>j.market===mktF);if(statusF)f=f.filter(j=>j.status===statusF);if(search){const q=search.toLowerCase();f=f.filter(j=>`${j.job_name} ${j.customer_name}`.toLowerCase().includes(q));}return f;},[jobs,mktF,statusF,search]);
  const pipeLF=filtered.filter(j=>['production_queue','in_production','active_installation','installation_complete'].includes(j.status)).reduce((s,j)=>s+n(j.total_lf),0);
  const sortByStart=(arr)=>[...arr].sort((a,b)=>(a.est_start_date||'9999').localeCompare(b.est_start_date||'9999'));
  const KANBAN_STS=['production_queue','in_production','active_installation','installation_complete','complete'];
