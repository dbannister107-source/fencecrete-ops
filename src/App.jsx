...
function ProductionPage({jobs,onRefresh}){
  ...
  const move=async(job,ns)=>{if(!editUnlocked)return;const u={status:ns};const today=new Date().toISOString().split('T')[0];if(ns==='active_install')u.active_install_date=today;if(ns==='fence_complete')u.fence_complete_date=today;if(ns==='fully_complete')u.fully_complete_date=today;if(ns==='closed')u.closed_date=today;try{const res=await fetch(`${SB}/rest/v1/jobs?id=eq.${job.id}`,{method:'PATCH',headers:H,body:JSON.stringify(u)});if(res.status!==200&&res.status!==204){const txt=await res.text();console.error('[Kanban] PATCH failed:',res.status,txt);setMoveToast({msg:`Move failed: ${txt}`,ok:false});return;}console.log('[Kanban] PATCH ok:',job.job_name,'→',SL[ns]);fireAlert('job_updated',{...job,...u});logAct(job,'status_change','status',job.status,ns);setMoveToast({msg:`Moved to ${SL[ns]}`,ok:true});onRefresh();}catch(e){console.error('[Kanban] Move error:',e);setMoveToast({msg:e.message||'Move failed',ok:false});}};
