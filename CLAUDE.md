...
**Four entry paths now wired up:**
- **Projects list folder column** — `＋` (blue plus) on jobs without a folder. Click writes job id to `sessionStorage['fc_open_sharepoint_modal']` and lets the row's natural onClick open the project. The EditPanel mount-effect consumes the sentinel and auto-launches the create modal.
- **EditPanel toolbar** — `📁 + Create SharePoint Folder` button next to `📂 Use Documents Tab` when no folder exists, market != OOS, user has edit permission. Click → `setShowSharepointModal(true)`.
...
**Phase A — Schema + synthetic backfill** (`fed9944`)
- 5 migrations applied: `job_pricing_lines`, `stage_weights` (universal defaults seeded — precast 65/20/15, SW 25/30/42/3, gates/options/permits/bonds 100% Complete), `job_stage_weights` (per-job overrides), `invoice_applications` (App #1/#2/#3 ledger), `invoice_application_lines` (per-stage breakdown), `v_effective_stage_weights` (override-then-default resolver view).
- 7 triggers wired: `trg_jpl_extended_total` (qty × price cache), `trg_jsw_validate_sum` (STATEMENT-level — refactored from CONSTRAINT TRIGGER mid-build for testability; rejects sums outside [0.99, 1.01]), `trg_set_app_number` (auto-increment per job), `trg_set_invoice_number` (auto-generates `INV-{job_number}-{NN}`, manual override flag preserves Virginia's custom values), `trg_compute_invoice_app_totals` (rolls up lines → header totals + retainage), `trg_post_to_invoice_entries_on_file` (draft→filed flips the App and INSERTs into the canonical `invoice_entries` ledger — sits ABOVE invoice_entries, no parallel ledger / no double-write risk; skips when `source_invoice_entry_id` is non-null), `trg_set_retainage_held` (maintains `jobs.retainage_held`; release App resets to 0).
- Wired previously unwired columns: `jobs.retainage_held` and `jobs.final_invoice_amount` (both schema placeholders for years; `retainage_held` now auto-maintained, `final_invoice_amount` reserved for Phase E "Mark Final" action).
- Synthetic backfill: 164 existing `invoice_entries` rows → 164 `invoice_applications` rows with `source_invoice_entry_id` set as provenance. 8 jobs picked up `retainage_held` values from the backfill. Synthetic apps carry no `application_lines` (no historical stage detail to reconstruct); UI renders them with a "Legacy Import" pill.
- 10 SQL assertions in `supabase/tests/billing_engine.sql`. All pass. CI auto-runs them on every push (existing GitHub Actions workflow from 2026-05-03).

**Phase B — Pricing editor** (`6bfbf9d`)
- New EditPanel "Pricing" sub-tab in the Money group, BEFORE Line Items (price book → contract scope → totals — clear dependency direction).
- `JobPricingEditor` component (`src/features/accounting/JobPricingEditor.jsx`, ~440 lines) — per-row editable pricing book mirroring the Excel "Original Contract Amounts" section. Columns: category / label / height / style / qty / unit (LF/EA/LS) / price/unit / labor/unit / tax_basis/unit / exempt / extended.
- Auto-seed on first open (decision C from planning round): when the table is empty for `(job_id, coId)`, fetch `job_line_items`, group by (category, height, style), generate candidate rows with HEIGHT_BASIS-derived tax_basis pre-filled (4'=$23, 5'=$24.75, 6'=$26, 7'=$27.50, 8'=$29.25, 9'=$30.50, 10'=$31.75 — extracted from inline App.jsx literals). Yellow banner "🌱 Generated from N line items — review and Save". No silent DB writes.
- `StageWeightsPanel` sub-component (~150 lines) — collapsible per-job override editor. Default state collapsed when no overrides exist; auto-expands when any override is present so users notice non-defaults. Per-category Edit / Override / Reset to defaults. Client-side validates sum=1.00 ± 0.01 before allowing save; server enforces too via the AFTER STATEMENT trigger.
- `registerSave` hook pattern matches `LineItemsEditor` — top-level Save button on EditPanel commits Pricing + Line Items + header fields in one click.
- Decoupled from `job_line_items` by design (decision #4) — saving Pricing does NOT write through to `job_line_items.contract_rate`. Phase D's calc engine surfaces drift if any.
- Permission gate: same `canEdit` as the rest of EditPanel.
- Drift cleanup: extracted `MoneyInput` from App.jsx → `src/shared/ui.js` (used by Pricing editor and 16 EditPanel money fields). Extracted `HEIGHT_BASIS` / `STYLE_BASIS` / `TAX_RATE` from 3 inline copies in App.jsx → `src/shared/billing/heightBasis.js` (used by `computedSalesTax` memo, NewProjectForm `computedSalesTax` memo, the bottom-of-Contract-tab Sales Tax breakdown, the Pricing editor, and the Phase C calc engine — single source of truth across all 5 surfaces).

**Phase C — Calc engine + SQL summary view** (`6f87b72`)
- `src/shared/billing/acctSheet.js` (~280 lines) — pure-JS calculation engine. 8 exported functions:
  - `apportionPmSubmission` — distributes PM bill sheet's aggregate per-stage LF (`labor_post_only`, `sw_foundation`, etc.) across pricing lines in the same category proportionally to contract qty share. Decision A from planning (least-surprising default; over-billing protection catches mistakes either way).
  - `mergeCumulativeQtys` — overlay manual cycleOverrides on top of apportionment. Used for single-stage gates/options/permits/bonds AND per-cell PM corrections.
  - `computeDraftCells` — core math; for each (pricing_line × stage), produces `cumulative_qty / prior_qty / current_qty / labor / tax_basis / tax / total` with over-billing warning when `cumulative > line.qty × 1.001`.
  - `aggregateDraftTotals` — rolls up to App-level `current_amount / current_retainage / net_due` (mirrors Phase A trigger so client-side preview matches what the DB will store after filing).
  - `computePerLineSummary` — per-pricing-line rollup including in-flight draft.
  - `buildLedger` — reverse-chron App ledger with `is_legacy_import` derived from `source_invoice_entry_id`.
  - `validateAcctSheet` — cross-cutting warnings (no-input draft, over-billing, missing labor/tax_basis splits).
  - `computeAcctSheet` — main entry point; orchestrates all of the above + computes contract summary.
- `src/shared/billing/__tests__/acctSheetFixtures.js` + `verify.mjs` — 48 assertions across 3 scenarios (Cycle 1: 100 LF Posts Only @ $98/LF → $6,509.43; Cycle 2: same LF advance to Posts & Panels → $2,002.90; tax-exempt: tax cell zeroed → $6,370). All numbers verified cell-for-cell against the Excel sample on 2026-05-05. **All 48 assertions PASS.** Run via `node src/shared/billing/__tests__/verify.mjs`. Not wired to CI yet (Vitest setup deferred per planning decision #4 until ≥2 features need a JS test runner).
- `v_acct_sheet_summary` SQL view — per-pricing-line rollup for Reports / ad-hoc queries. Distinct name from the legacy `v_acct_sheet_lines` view (which reads from `job_line_items` and predates this work — left in place untouched per CLAUDE.md no-destructive-ops rule, deprecated by this build but not dropped).
- 2 new DB tests appended to `billing_engine.sql` (Tests 11+12). DB suite is now 12/12 PASS.

**Phase D — Accounting tab UI** (`ceab60d`)
- 4 new components in `src/features/accounting/`: `AccountingTab.jsx` (~370 lines, top-level orchestrator), `ContractSummaryCard.jsx` (~100 lines, 5 money tiles + progress bar), `DraftTable.jsx` (~230 lines, per-line × per-stage editable table), `AppLedger.jsx` (~160 lines, reverse-chron history).
- Mounted as the new "Accounting" tab in the EditPanel Money group, AFTER Change Orders. Final Money group order: Contract & Billing → Pricing → Line Items → Totals → Change Orders → **Accounting**.
- Layout: Contract Summary card (Contract / Billed / Pending / Balance / Retainage tiles + progress bar) → Warnings banner (when computeAcctSheet emits any) → Current Bill Draft section (PM submission selector + invoice date + notes + DraftTable + File Invoice button) → App Ledger (reverse-chron with status pills + Release Retainage button when `retainage_held > 0`).
- DraftTable behavior: multi-stage categories (precast, sw) read-only — driven by PM bill sheet apportionment; user adjusts on PM Bill Sheet, not here. Single-stage categories (wi_gate, option, permit, bond, other) editable — checkbox toggles 0 ↔ full qty; click qty to type partial. Over-billing cells: red border + ⚠ marker.
- File Invoice flow safety: 3 steps with rollback safety. (1) Insert header `status='draft'` (post trigger only fires on draft→filed); (2) Insert app_lines; (3) Flip `status='filed'` → Phase A trigger posts to `invoice_entries`, `ytd_invoiced` cascades. If step 2 fails after step 1: orphan draft App with no `invoice_entries` side effect. Phase E will add a maintenance UI for these (rare).
- Release Retainage flow: header-only (no app_lines, since the compute trigger fires on lines not headers). `current_amount / net_due / retainage_to_date` set manually alongside `status='filed'`. `trg_set_retainage_held` zeroes `jobs.retainage_held` because `is_retainage_release=true`. Single button + native confirm dialog (decision #3 from planning — mechanical and fast for Virginia's workflow).
- File Invoice gating: `canEdit=false` hides the button; over-billing warning disables it ("Resolve first"); draft total ≤ $0 disables it ("No billable activity"); missing labor/tax_basis split is warn-only (cells with no split just don't post; user can still file partial).
...
- Phase E candidates (deferred — none scheduled): orphan draft App cleanup UI, App row click-through modal showing per-stage detail, "Mark Final" action that writes `jobs.final_invoice_amount`, PDF export of the Acct Sheet matching Virginia's preferred Excel layout, optional sync of pricing → `job_line_items.contract_rate` (decision was decoupled by design, but the calc engine now surfaces drift so a sync action is ready if requested).
- **Cutover is operational, not engineering.** Virginia uses the new tab when ready; the legacy AR review path on BillingPage continues to work in parallel. Both paths post to the same `invoice_entries` ledger so `jobs.ytd_invoiced` stays correct regardless of which path is taken — but double-billing the same submission via both paths would create duplicate entries. The amber badge on BillingPage is the heads-up. Once Virginia's confident, deprecating the direct-post-from-AR path is a future cleanup (1-2 hours).
- All builds green, all CI green on every commit.
...
*Last updated: May 7, 2026 — **Bill Draft drawer on the Billing page (Submissions tab).** Per Amiee, surfaced the per-project "Current Bill Draft" section from `AccountingTab` directly on the Billing → Submissions tab so AR doesn't have to drill into each project's EditPanel → Money → Accounting tab one by one. New `🧾 Bill Draft` button on every Pending row (red outline, opens drawer in edit mode with full File Invoice flow) and `🧾 View Draft` on Reviewed rows (gray, read-only). No Bill rows hide the button. Drawer is a right-side slideout `min(1100px, 95vw)` so the 9-column DraftTable renders without horizontal squish; mounts the existing `<AccountingTab>` component as-is (same triggers, same audit trail, same ledger). After File Invoice the drawer stays open so AR can verify the new App in the AppLedger section below the Draft. The legacy "Enter Invoice" button stays alongside during transition — both paths coexist; AR picks. Code: `+1` state, `+1` inline `BillDraftDrawer` component (~50 lines), `+1` button on Pending/Reviewed rows, `+1` mount near the existing modal mounts (commit `65dd77f`). No DB migration; AccountingTab itself unchanged.*

*Earlier May 7, 2026 — **`sync_job_aggregates_from_line_items` trigger now propagates dollar fields too.** Pre-existing gap since the Pricing Book retirement (2026-05-05): the trigger synced LF aggregates and primary style/color/height but not the dollar fields. Surfaced by Amiee's Woodhavyn job (26H033, tax-exempt) — line items totaled $33,810 but `jobs.net_contract_value` was $0 and `adj_contract_value` showed $0 on the Totals strip. Trigger now also computes `net_contract_value = SUM(line_value)` excluding bonds/permits, plus `pp_bond_amount`, `maint_bond_amount`, `permit_amount` per fence_type. `trg_recalc_adj_contract` automatically picks up the new net + bonds + permit and computes `adj = net + COs + permit + bonds + sales_tax` correctly. **Safety guard:** dollar aggregates use `COALESCE(NULLIF(sum, 0), existing)` so jobs with line_value=0 line items (9 legacy jobs from pre-Pricing-Book imports — Cane Island $316k, FBC MUD 250 $169k, etc.) keep their stored values when Amiee touches a line item. Migration: `20260507_sync_job_aggregates_propagate_money_fields.sql`. Verified: Woodhavyn now `total_lf=325`, `net=$31,200`, `pp_bond=$805`, `maint_bond=$805`, `permit=$1,000`, `adj=$33,810` (tax-exempt → no sales tax). 113 net_contract_value drift cases remain across the dataset; not auto-backfilled (surface as Amiee touches each).*
