...
const DEF_VIS=['job_number','job_name','status','pm','sales_rep','primary_fence_type','style','color','total_lf_precast','lf_single_wythe','lf_wrought_iron','number_of_gates','market','adj_contract_value','left_to_bill','pct_billed','contract_date','est_start_date','fence_addons','notes'];
...
function ProjectsPage({jobs,onRefresh,openJob,refreshKey=0,onNav}){
  ...
  const[visCols,setVisCols]=useState(()=>{try{
    const s=localStorage.getItem('fc_vis_cols_v2');
    if(!s)return DEF_VIS;
    ...
    localStorage.setItem('fc_vis_cols_v2',JSON.stringify(saved));
    return saved;
  }catch(e){return DEF_VIS;}});const[showCols,setShowCols]=useState(false);
  useEffect(()=>{try{localStorage.setItem('fc_vis_cols_v2',JSON.stringify(visCols));}catch(e){}},[visCols]);
