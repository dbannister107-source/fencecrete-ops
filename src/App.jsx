  ...
  return <div style={{position:'fixed',inset:0,zIndex:700,background:'rgba(0,0,0,0.6)',display:'flex',flexDirection:'column',justifyContent:'flex-end'}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{background:'#1A1A1A',borderTopLeftRadius:20,borderTopRightRadius:20,maxHeight:'88vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{padding:'20px 20px 16px',borderBottom:'1px solid rgba(255,255,255,0.07)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <img src="/logo.png" alt="Fencecrete" style={{height:28,width:'auto',filter:'brightness(0) invert(1)',opacity:0.92}}/>
      ...
      <div style={{overflowY:'auto',padding:'8px 10px 80px',flex:1}}>{NAV_GROUPS.map(g=><div key={g.label||'top'}>
        {g.label&&<div style={{fontSize:9,color:'rgba(255,255,255,0.28)',textTransform:'uppercase',letterSpacing:'0.12em',fontWeight:600,padding:'14px 10px 4px'}}>{g.label}</div>}
        {g.items.map(ni=>{const _svg=_micons[ni.key]||'';return <button key={ni.key} onClick={()=>{setPage(ni.key);onClose();}} style={{display:'flex',alignItems:'center',gap:12,width:'100%',padding:'13px 10px',marginBottom:1,borderRadius:8,border:'none',background:page===ni.key?'rgba(138,38,29,0.15)':'transparent',color:page===ni.key?'#FFFFFF':'rgba(255,255,255,0.55)',fontSize:14,fontWeight:page===ni.key?500:400,cursor:'pointer',textAlign:'left',borderLeft:page===ni.key?'2px solid #8A261D':'2px solid transparent'}}>
